...
export function computeFlags(db, opts = {}) {
  ...
  try {
    ...
    if (rec.enabled && rec.summary?.missing > 0) {
      ...
      const fmtL = (l) => (l.length === 7 ? l : `${Number(l.slice(8, 10))}.${Number(l.slice(5, 7))}`);
      ...
      flags.push({
        ...
        text: `כסף שנסלק בקארדקום לא הגיע לבנק: מכירות ${span}, ${r(rec.summary.missingAmount).toLocaleString('he-IL')} ₪ שהיו צפויים בחשבון ולא נמצאו. לבדוק מול חברת הסליקה`,
...
export function computeReconcile(db, days = 45) {
  ...
  // Monthly mode compares whole months, so the window must start on a 1st.
  ...
  `).all(from);
  // The first month of sales data is usually partial (sync started mid-month);
  // in monthly mode it would look like a huge over-payment. Drop it.
  if (settlement.mode === 'monthly' && sales.length) {
    const firstEver = db.prepare(`SELECT MIN(date) AS d FROM cardcom_sales`).get().d;
    if (firstEver && firstEver.slice(8, 10) !== '01') {
      const cut = firstEver.slice(0, 7);
      sales = sales.filter((s) => s.date.slice(0, 7) !== cut);
    }
  }
...
export function computeBriefing(db) {
  ...
  if (cf) {
    ...
    out.numbers.dip = dip ? { date: dip.date, amount: dip.projected } : null;
  }
  ...
  if (cc.enabled) {
    ...
    if (recon) {
      ...
      if (s.missing > 0) {
        tone = 'action';
        const miss = recon.rows.filter((r) => r.status === 'missing');
        text += ` ${ils(s.missingAmount)} ממכירות ${miss.map((r) => lbl(r.label)).join(', ')} עדיין לא בבנק.`;
      } else if (s.partial > 0) {
        tone = 'attention';
