...
export function computeBriefing(db) {
  ...
  if (cc.enabled) {
    ...
    try { recon = computeReconcile(db, 45); } catch { /* optional */ }
    let text = y.count > 0
    ...
    if (recon) {
      ...
      if (s.missing > 0) {
        ...
        text += ` ${ils(s.missingAmount)} ממכירות ${miss.map((r) => dm(r.label)).join(', ')} עדיין לא בבנק.`;
      } else if (s.partial > 0) {
        tone = 'attention';
        text += ` באחד הימים הזיכוי מהסליקה הגיע בסכום שונה מהצפוי.`;
      } else if (matched.length) {
        const lastLanded = matched.map((r) => r.label).sort().pop();
        text += ` הכסף של כל הימים עד ${dm(lastLanded)} כבר בבנק.`;
      }
    }
