...
export function computeQuality(db, today = israelToday(), opts = {}) {
  ...
  try {
    ...
    const bankSync = db.prepare(`SELECT MAX(ts) ts FROM sync_log WHERE source LIKE 'financy%' AND ok=1`).get().ts;
    const bankHasRows = db.prepare(`SELECT COUNT(*) n FROM bank_transactions WHERE account_type='CHECKING'`).get().n > 0;
    const ccLast = db.prepare(`SELECT MAX(ts) ts FROM sync_log WHERE source='cardcom' AND ok=1`).get().ts;
    findings.push(...Q.checkFreshness({ today, bankLastSync: bankSync, bankHasRows, cardcomEnabled: cc.enabled, cardcomLastSync: ccLast, daysBetween }));
  } catch { /* optional */ }
