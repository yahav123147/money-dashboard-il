...
import * as Q from '../scripts/lib/quality.mjs';
...
export function computeOverview(db) {
  ...
  const lastFinancy = utcIso(db.prepare(`
    SELECT MAX(ts) AS ts FROM sync_log WHERE source='financy' AND ok=1
  `).get().ts);
...
export function computeRecurring(db, today = israelToday(), opts = {}) {
  const rec = opts.recurring || recurring; // tests pass their own items instead of touching the config file
  const rows = db.prepare(`
    SELECT date, amount, counterparty, bucket FROM bank_transactions
  ...
  `).all(today);
  const ignore = new Set((rec.ignore || []).map(normalizeName));
  const manual = new Set((rec.items || []).map((it) => normalizeName(it.name)));
  // A manual item owns its bucket: "משכורות צוות" (team) stands in for every
  // learned salary line, "מע\"מ" for the learned VAT debit, and so on.
  // Otherwise the same money is projected twice.
  const ownedBuckets = new Set((rec.items || []).map((it) => it.bucket).filter(Boolean));
  const learned = detectRecurring(rows, { today })
    .filter((it) => !ignore.has(normalizeName(it.name)) && !manual.has(normalizeName(it.name)))
    .filter((it) => !(it.bucket && ownedBuckets.has(it.bucket)));
  return { learned, manual: rec.items || [], ignore: rec.ignore || [] };
}
...
export function computeCashflow(db, today = israelToday(), horizon = 30) {
  ...
  // Which bank each projected item belongs to. Actual rows carry their
  // account; a recurring item belongs to the bank that has been paying it.
  const accountProvider = new Map(db.prepare(`SELECT id, provider FROM accounts`).all().map((a) => [a.id, a.provider]));
  const nameProvider = new Map();
  for (const row of db.prepare(`
    SELECT counterparty, provider, COUNT(*) n FROM bank_transactions
    WHERE account_type='CHECKING' AND counterparty IS NOT NULL AND counterparty != '' AND date >= ?
    GROUP BY counterparty, provider ORDER BY n DESC
  `).all(shiftDate(today, -400))) {
    const k = normalizeName(row.counterparty);
    if (!nameProvider.has(k)) nameProvider.set(k, row.provider);
  }
  const primaryProvider = db.prepare(`
    SELECT provider, SUM(balance * CASE WHEN currency='USD' THEN ? ELSE 1 END) s
    FROM accounts WHERE type='CHECKING' GROUP BY provider ORDER BY s DESC LIMIT 1
  `).get(rate)?.provider || null;
  const providerForName = (name) => nameProvider.get(normalizeName(name || '')) || primaryProvider;
  ...
  // Card bills, per bank. The bank schedules part of next month's charge as a
  // future row (the amount accrued so far); the rest of the month's bill is
  // still coming. Project the difference between what a month usually costs
  // on this bank's cards and what is already on the calendar, on the day the
  // charge historically lands. Without this, a card-heavy account looks flat.
  const cardProjections = [];
  try {
    const hist = db.prepare(`
      SELECT provider, month, SUM(-amount) total FROM bank_transactions
      WHERE bucket='card_settlement' AND amount < 0 AND account_type='CHECKING'
        AND month >= ? AND month <= ?
      GROUP BY provider, month ORDER BY provider, month
    `).all(shiftMonth(curMonth, -4), curMonth);
    const byProv = {};
    for (const h of hist) (byProv[h.provider] = byProv[h.provider] || []).push(h.total);
    const dayRows = db.prepare(`
      SELECT provider, CAST(substr(date, 9, 2) AS INTEGER) d, SUM(-amount) s FROM bank_transactions
      WHERE bucket='card_settlement' AND amount < 0 AND account_type='CHECKING' AND month >= ?
      GROUP BY provider, d ORDER BY s DESC
    `).all(shiftMonth(curMonth, -6));
    const mainDay = {};
    for (const row of dayRows) if (!mainDay[row.provider]) mainDay[row.provider] = row.d;
    const scheduled = {}; // provider|month already on the calendar
    for (const row of futureRows) {
      if (row.bucket !== 'card_settlement' || row.amount >= 0) continue;
      const k = (accountProvider.get(row.account_id) || primaryProvider) + '|' + row.date.slice(0, 7);
      scheduled[k] = (scheduled[k] || 0) + -row.amount;
    }
    const median = (xs) => { const a = [...xs].sort((p, q) => p - q); const m = a.length >> 1; return a.length ? (a.length % 2 ? a[m] : (a[m - 1] + a[m]) / 2) : 0; };
    // What has already accrued on that bank's cards since the last bill. Near
    // the charge date this IS the bill; far from it the cycle has barely
    // started and the historical average is the better guess.
    const accrued = {};
    for (const a of db.prepare(`SELECT provider, COALESCE(SUM(balance), 0) s FROM accounts WHERE type='CARD' AND COALESCE(currency,'ILS')='ILS' GROUP BY provider`).all()) accrued[a.provider] = a.s;
    for (const [prov, totals] of Object.entries(byProv)) {
      if (totals.length < 2 || !mainDay[prov]) continue;
      const expect = median(totals.slice(-3));
      for (let m = 0; m <= Math.ceil(horizon / 28); m++) {
        const ym = shiftMonth(curMonth, m);
        const lastDay = Number(shiftDate(`${shiftMonth(ym, 1)}-01`, -1).slice(8, 10));
        const date = `${ym}-${String(Math.min(mainDay[prov], lastDay)).padStart(2, '0')}`;
        if (date <= today || date > shiftDate(today, horizon)) continue;
        const daysOut = daysBetween(today, date);
        const near = daysOut <= 10 && (accrued[prov] || 0) > 0;
        const expected = near ? accrued[prov] : expect;
        const remaining = expected - (scheduled[prov + '|' + ym] || 0);
        if (remaining < 1000) continue;
        cardProjections.push({
          date,
          name: `חיוב כרטיסי אשראי (${providerLabel(prov)}, ${near ? 'לפי מה שנצבר בכרטיס' : 'לפי הממוצע החודשי'})`,
          amount: -r(remaining), provider: prov, projectedCard: true,
        });
      }
    }
  } catch { /* optional */ }
  const cardByDate = new Map();
  for (const c of cardProjections) {
    if (!cardByDate.has(c.date)) cardByDate.set(c.date, []);
    cardByDate.get(c.date).push(c);
  }
  const projectedCardProviders = new Set(cardProjections.map((c) => c.provider));

  // Expected CardCom settlements still inside their window.
  let expectedSettlements = [];
  try {
    const recon = computeReconcile(db, 120, shiftDate(today, horizon));
    if (recon.enabled) {
      // Only acquirers whose money has ever landed in this account. AmEx or
      // PayPal payouts that go elsewhere are a reconciliation finding, not
      // cash this account will see.
      const landsHere = new Set(Object.entries(recon.byAcquirer || {}).filter(([, a]) => a.received > 0).map(([k]) => k));
      expectedSettlements = recon.rows
        .filter((x) => x.status === 'pending' && landsHere.has(x.acquirer))
        .map((x) => ({
          date: x.windowFrom <= today ? shiftDate(today, 1) : x.windowFrom,
          name: `זיכוי ${acquirerLabel(x.acquirer)} (${x.label.length === 7 ? 'מכירות ' + hebMonthShort(shiftDate(`${x.label}-01`, -1).slice(0, 7)) : 'מכירות ' + x.label})`,
          amount: x.expected, settlement: true,
        }));
    }
  } catch { /* optional */ }
  ...
  for (let i = 1; i <= horizon; i++) {
    ...
    const items = (futureByDate.get(date) || []).map((row) => ({
      ...
      amount: row.amount,
      actual: true,
      provider: accountProvider.get(row.account_id) || primaryProvider,
    }));
    for (const it of recurringItems) {
      ...
      if (it.bucket === 'card_settlement' && projectedCardProviders.has(it.provider || providerForName(it.name))) continue;
      ...
      items.push({ name: it.name, amount: it.amount, learned: !!it.learned, provider: it.provider || providerForName(it.name) });
    }
    for (const c of cardByDate.get(date) || []) items.push({ name: c.name, amount: c.amount, provider: c.provider, projectedCard: true });
    for (const s of settleByDate.get(date) || []) items.push({ name: s.name, amount: s.amount, settlement: true, provider: providerForName('קארדקום') });
    const net = items.reduce((sum, it) => sum + it.amount, 0);
    running += net;
    days.push({ date, items, net: r(net), projected: r(running) });
  }

  // Per bank: opening balance, where it ends, and its own low point. The
  // banks sum to the totals above, so a tile per bank and the headline
  // number can never tell different stories.
  const byProvider = {};
  for (const a of db.prepare(`
    SELECT provider, COALESCE(SUM(balance * CASE WHEN currency='USD' THEN ? ELSE 1 END), 0) s, COUNT(*) n
    FROM accounts WHERE type='CHECKING' GROUP BY provider
  `).all(rate)) {
    byProvider[a.provider] = { provider: a.provider, balance: r(a.s), accounts: a.n, projected: r(a.s), dip: null, items: [] };
  }
  for (const d of days) {
    const seen = new Set();
    for (const it of d.items) {
      const p = it.provider || primaryProvider;
      if (!byProvider[p]) byProvider[p] = { provider: p, balance: 0, accounts: 0, projected: 0, dip: null, items: [] };
      byProvider[p].projected = r(byProvider[p].projected + it.amount);
      byProvider[p].items.push({ date: d.date, name: it.name, amount: r(it.amount) });
      seen.add(p);
    }
    for (const p of seen) {
      const b = byProvider[p];
      if (!b.dip || b.projected < b.dip.projected) b.dip = { date: d.date, projected: b.projected };
    }
  }
  for (const b of Object.values(byProvider)) {
    b.items.sort((x, y) => Math.abs(y.amount) - Math.abs(x.amount));
    b.next = b.items.length ? [...b.items].sort((x, y) => x.date.localeCompare(y.date) || Math.abs(y.amount) - Math.abs(x.amount))[0] : null;
    b.biggest = b.items[0] || null;
    b.items = b.items.slice(0, 8);
  }
  ...
  return {
    ...
    byProvider, primaryProvider,
    warnIls,
  };
}

// One row per bank for the status screen: how much is there, where it is
// heading in the horizon, and the item that moves it most.
const PROVIDER_LABELS = {
  leumi: 'בנק לאומי', hapoalim: 'בנק הפועלים', discount: 'בנק דיסקונט', mizrahi: 'מזרחי טפחות',
  beinleumi: 'הבינלאומי', mercantile: 'מרכנתיל', jerusalem: 'בנק ירושלים', otsarhahayal: 'אוצר החייל',
  massad: 'בנק מסד', pagi: 'פאג"י', union: 'בנק אגוד', oneZero: 'וואן זירו', yahav: 'בנק יהב',
};
export function providerLabel(p) { return PROVIDER_LABELS[p] || p || 'בנק'; }
export function computeBanks(db, today = israelToday(), horizon = 30) {
  const cf = computeCashflow(db, today, horizon);
  const banks = Object.values(cf.byProvider || {})
    .map((b) => ({ ...b, label: providerLabel(b.provider), net: r(b.projected - b.balance) }))
    .sort((a, b) => b.balance - a.balance);
  return { today, horizon, banks, total: cf.startBalance, totalProjected: cf.endProjected };
}
...
export function computeReconcile(db, days = 45, horizonTo = null) {
  const cfg = cardcomConfig();
  ...
  const credits = db.prepare(`
    ...
    WHERE account_type='CHECKING' AND currency='ILS' AND amount > 0 AND date >= ?
      -- a refund from the issuer of OUR card carries the same issuer name as an
      -- acquirer settlement; it is our own money coming back, never a customer's
      AND COALESCE(bucket, '') != 'card_settlement'
    ORDER BY date
  `).all(from);
  const out = reconcile({ sales, credits, settlement: { ...settlement, keywords }, today, horizonTo });
...
export function computeBriefing(db) {
  ...
  // 0. Data quality first: a forecast built on a known-bad input must not
  //    drive a "postpone expenses" line, however confident the arithmetic.
  let quality = null;
  try { quality = computeQuality(db, today); } catch { /* optional */ }
  const forecastTrusted = !quality || quality.confidence !== 'low';
  const forecastPartial = !!quality && quality.confidence === 'medium';
  ...
  if (cf) {
    ...
    const dip = forecastTrusted ? cf.dip : null;
    let text = `בבנק ${ilsK(bal)}.`;
    if (!forecastTrusted) {
      text += ' התחזית ל-30 יום מושהית: ' + quality.confidenceText.replace('התחזית ', '').replace('עד שיטופלו הפריטים למטה', 'עד שיטופלו הפריטים ברשימה') + '.';
    } else if (dip && dip.date !== shiftDate(today, 1) && Math.abs(dip.projected - bal) > 500) {
    ...
    if (forecastPartial) text += ' התחזית חלקית: ' + quality.confidenceText.replace('התחזית סבירה; ', '') + '.';
    // A partial forecast never gets the action tone: the number may be missing an inflow.
    out.lines.push({ key: 'cash', label: 'כסף', text, tone: dip && dip.projected < 0 ? (forecastPartial ? 'attention' : 'action') : 'plain' });
    out.numbers.balance = bal;
    out.numbers.dip = dip ? { date: dip.date, amount: dip.projected, belowWarn: !!dip.belowWarn } : null;
    out.numbers.endProjected = forecastTrusted ? cf.endProjected : null;
  ...
  for (const f of flags) {
    switch (f.type) {
      ...
      case 'cash_dip':
        if (!forecastTrusted) break; // the dip is an artefact until the quality items are fixed
        if (forecastPartial) { out.todo.week.push({ key: 'dip', text: f.text.replace('תזרים: ', '') + ' (תחזית חלקית)', action: 'קודם לעדכן את הנתונים החסרים, ואז להחליט' }); break; }
  ...
  // 5. Data quality: what could make the numbers above wrong. Fixes go to
  //    the week column, settings to "when you have time"; info stays on the
  //    quality panel only. Never the "today" column: nothing is overdue.
  try {
    const q = quality || computeQuality(db, today);
    out.numbers.quality = { verdict: q.verdict, confidence: q.confidence, confidenceText: q.confidenceText, counts: q.counts };
    for (const f of q.findings) {
      if (f.severity === 'fix') out.todo.week.push({ key: 'q:' + f.key, text: f.title + ': ' + f.text, action: f.action });
      else if (f.severity === 'warn') out.todo.later.push({ key: 'q:' + f.key, text: f.title + ': ' + f.text, action: f.action });
    }
  } catch { /* optional */ }
...
// --- Data quality --------------------------------------------------------------
// The deterministic layer that protects the forecast from bad inputs. Each
// check lives in scripts/lib/quality.mjs; this gathers what they need.
export function computeQuality(db, today = israelToday(), opts = {}) {
  const findings = [];
  const m3 = shiftDate(today, -92);

  // Recurring: manual vs learned duplicates, and manual items the bank no longer shows.
  try {
    const rec = computeRecurring(db, today, opts);
    findings.push(...Q.checkRecurringDuplicates({ manual: rec.manual, learned: rec.learned }));
    const seen = db.prepare(`
      SELECT DISTINCT counterparty, bucket FROM bank_transactions
      WHERE account_type='CHECKING' AND currency='ILS' AND date >= ? AND date <= ?
    `).all(m3, today);
    findings.push(...Q.checkStaleManual({
      manual: rec.manual,
      seenNames: new Set(seen.map((x) => normalizeName(x.counterparty || ''))),
      seenBuckets: new Set(seen.map((x) => x.bucket).filter(Boolean)),
    }));
  } catch { /* optional */ }

  // Sales: acquirers that never land here, partial first month, stale sync.
  const cc = cardcomConfig();
  if (cc.enabled) {
    try {
      const recon = computeReconcile(db, (cc.settlement?.mode === 'monthly') ? 180 : 60);
      findings.push(...Q.checkAcquirersNeverLand({ byAcquirer: recon.byAcquirer || {}, labels: acquirerLabel }));
      const first = db.prepare('SELECT MIN(date) d FROM cardcom_sales').get().d;
      findings.push(...Q.checkPartialFirstMonth({ firstSaleDate: first, mode: cc.settlement?.mode }));
    } catch { /* optional */ }
  }

  // Tax: debit days vs settings, VAT estimate vs what was paid.
  try {
    const y1 = shiftDate(today, -366);
    const days = (bucket) => db.prepare(`
      SELECT date FROM bank_transactions
      WHERE account_type='CHECKING' AND currency='ILS' AND amount < 0 AND bucket=? AND date >= ? AND date <= ?
    `).all(bucket, y1, today).map((x) => Number(x.date.slice(8, 10)));
    const entityType = settings.entityType ?? null;
    findings.push(...Q.checkTaxDebitDays({
      vatDays: entityType === 'patur' ? [] : days('tax_vat'),
      advanceDays: days('tax_advance'),
      vatDueDay: settings.vatDueDay ?? 15,
      advanceDueDay: settings.advanceDueDay ?? 15,
    }));
    if (entityType && entityType !== 'patur') {
      const period = vatPeriodMonths(db, { entityType });
      if (period) {
        const periods = [];
        let cursor = periodFor(shiftDate(`${today.slice(0, 7)}-01`, -1).slice(0, 7), period.months);
        for (let i = 0; i < 4; i++) {
          const v = vatForRange(db, cursor.from, cursor.to);
          const paid = r(-db.prepare(`
            SELECT COALESCE(SUM(amount), 0) s FROM bank_transactions
            WHERE account_type='CHECKING' AND currency='ILS' AND bucket='tax_vat' AND amount < 0 AND date >= ? AND date <= ?
          `).get(shiftDate(cursor.due, -10), shiftDate(cursor.due, 12)).s);
          periods.push({ ...cursor, net: v.net, paid });
          cursor = periodFor(shiftDate(`${cursor.from}-01`, -1).slice(0, 7), period.months);
        }
        findings.push(...Q.checkVatEstimate({ periods }));
      }
    }
  } catch { /* optional */ }

  // Bank hygiene: duplicate pending rows, unclassified share, FX rate, freshness.
  try {
    const dupes = db.prepare(`
      SELECT COUNT(*) - COUNT(DISTINCT account_id || '|' || date || '|' || amount || '|' || COALESCE(counterparty,'')) n,
             COALESCE(SUM(amount), 0) - COALESCE((SELECT SUM(a) FROM (SELECT amount a FROM bank_transactions WHERE status='PENDING' AND date > ? GROUP BY account_id, date, amount, counterparty)), 0) amt
      FROM bank_transactions WHERE status='PENDING' AND date > ?
    `).get(today, today);
    findings.push(...Q.checkPendingDuplicates({ dupes: dupes.n, amount: dupes.amt }));
    const u = db.prepare(`
      SELECT
        COALESCE(SUM(CASE WHEN bucket_group='unclassified' THEN -amount END), 0) unc,
        COALESCE(SUM(CASE WHEN bucket_group IN ('expense','unclassified') THEN -amount END), 0) total,
        COALESCE(SUM(CASE WHEN bucket_group='unclassified' THEN 1 END), 0) n
      FROM bank_transactions
      WHERE account_type='CHECKING' AND currency='ILS' AND amount < 0 AND date >= ? AND date <= ?
    `).get(m3, today);
    findings.push(...Q.checkUnclassifiedShare({ unclassified: u.unc, total: u.total, count: u.n }));
    const hasUsd = db.prepare(`SELECT COUNT(*) n FROM accounts WHERE currency='USD'`).get().n > 0
      || db.prepare(`SELECT COUNT(*) n FROM bank_transactions WHERE currency='USD'`).get().n > 0;
    findings.push(...Q.checkFxRate({ hasUsd, usdToIls: settings.usdToIls }));
    const bankSync = db.prepare(`SELECT MAX(ts) ts FROM sync_log WHERE source='financy' AND ok=1`).get().ts;
    const ccLast = db.prepare(`SELECT MAX(ts) ts FROM sync_log WHERE source='cardcom' AND ok=1`).get().ts;
    findings.push(...Q.checkFreshness({ today, bankLastSync: bankSync, cardcomEnabled: cc.enabled, cardcomLastSync: ccLast, daysBetween }));
  } catch { /* optional */ }

  // A "second account" setting that a later connection made stale.
  try {
    const providers = db.prepare(`SELECT DISTINCT provider p FROM bank_transactions WHERE provider IS NOT NULL AND provider != ''`).all().map((x) => x.p);
    const bucket = settings.secondAccountBucket || null;
    const transfers = bucket ? r(-db.prepare(`
      SELECT COALESCE(SUM(amount), 0) s FROM bank_transactions
      WHERE amount < 0 AND bucket = ? AND month >= ?
    `).get(bucket, today.slice(0, 4) + '-01').s) : 0;
    findings.push(...Q.checkSecondAccountStale({ secondAccountBucket: bucket, providers, transfersIls: transfers }));
  } catch { /* optional */ }

  const order = { fix: 0, warn: 1, info: 2 };
  findings.sort((a, b) => order[a.severity] - order[b.severity]);
  return { date: today, findings, ...Q.summarizeQuality(findings) };
}
