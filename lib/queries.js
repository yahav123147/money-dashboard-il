...
export function computeRecurring(db, today = israelToday()) {
  ...
  const ownedBuckets = new Set((recurring.items || []).map((it) => it.bucket).filter(Boolean));
...
export function computeQuality(db, today = israelToday()) {
  const findings = [];
  ...
  try {
    ...
    const bankLast = db.prepare(`SELECT MAX(date) d FROM bank_transactions WHERE date <= ? AND status != 'PENDING'`).get(today).d;
    const ccLast = db.prepare(`SELECT MAX(ts) ts FROM sync_log WHERE source='cardcom' AND ok=1`).get().ts;
    findings.push(...Q.checkFreshness({ today, bankLastDate: bankLast, cardcomEnabled: cc.enabled, cardcomLastSync: ccLast, daysBetween }));
  } catch { /* optional */ }
